...
### 3.  Simple correlation test with sample size 
sample size refers to the mean number of individuals sampled for the GD estimates

-   $H_o$ No correlation between GD and sample size size
-   $H_1$ GD increases with sample size

```{r}
ggplot(data=gen_clus) + geom_point(aes(x=sp, y=sample.size))
```
...
# Remove data with more than 200 individuals
```{r}
#gen_clus <- gen_clus[!gen_clus$sample.size > 100,]
...
Remove outliers in cluster dataset using the boxplot 
...
abline(h=mean(gen_clus$DP_log, na.rm=T), lty=2)

boxplot(gen_clus$ho~gen_clus$sp, col=no_green[names(no_green) %in% unique(gen_clus$sp)] , xlab=NA,  las=2, ylab="Ho", na.action = na.omit)
abline(h=mean(gen_clus$ho, na.rm=T), lty=2)

boxplot(gen_clus$he~gen_clus$sp, col=no_green[names(no_green) %in% unique(gen_clus$sp)], las=2, xlab=NA,  ylab="He", na.action = na.omit)
abline(h=mean(gen_clus$he, na.rm=T), lty=2)

boxplot(log10(gen_clus$n.allele)~gen_clus$sp, col=no_green[names(no_green) %in% unique(gen_clus$sp)], las=2, xlab=NA,cex=1,  ylab="log10(n.allele)" , na.action = na.omit)
...
```{r, results='asis'}
par(mar=c(9,2,1,2))
boxplot(log10(gen_clus$sample.size)~gen_clus$sp,   xlab=NA, las=2, col=no_green[names(no_green) %in% unique(gen_clus$sp)], ylab="log10(sample size")
abline(h=mean(log10(gen_clus$sample.size), na.rm=T), lty=2)

boxplot(log10(gen_count$sample.size) ~gen_count$sp,   xlab=NA, las=2, col=no_green[names(no_green) %in% unique(gen_count$sp)], ylab="log10(sample size)")
abline(h=mean(log10(gen_count$sample.size), na.rm=T), lty=2)

boxplot(gen_clus$no.of.loci~gen_clus$sp, col=no_green[names(no_green) %in% unique(gen_clus$sp)] , xlab=NA,  las=2, ylab="No. of loci")
abline(h=mean(gen_clus$no.of.loci, na.rm=T), lty=2)

boxplot(gen_count$no.of.loci~gen_count$sp, col=no_green[names(no_green) %in% unique(gen_count$sp)] , xlab=NA,  las=2, ylab="No. of loci")
...
## GD and DP Relationship within species (Figure S2)
...
for (i in 1:length(df_sp)){
  ...
  p_Ho <- ggscatter(df, x = "DP_log", y = "ho",  xlab="log10(PD)", size= "sample.size_log", main="",
              #  add = "reg.line",  # Add regressin line
               # add.params = list(color = "black", linecolor = "black"), # Customize reg. line
                conf.int = F # Add confidence interval
) + stat_cor(method = "p", alternative = "greater", color="black", digits=2,  label.y=1, aes(
...
p_He <- ggscatter(df, x = "DP_log", y = "he",  xlab="log10(PD)",  size= "sample.size_log", main="",
                 # add = "reg.line",  # Add regressin line
                #  add.params = list(color = "black"), # Customize reg. line
                  conf.int = F # Add confidence interval
) + stat_cor(method = "p", alternative = "greater", color="black", label.y=1, digits=2,aes(
               label =paste(..r.label.., ..p.label.., cut(..p.., 
                                              breaks = c(-Inf, 0.0001, 0.001, 0.01, 0.05, Inf),
                                              labels = c("'****'", "'***'", "'**'", "'*'", "''")), 
                            sep = "~")), 
...
p_mna <- ggscatter(df, x = "DP_log", y = "n.allele_log", size= "sample.size_log", ylab="log10(Ar)", xlab="log10(PD)", main="",
...
plot_sp[[i]] <- ggarrange(p_Ho, p_He, p_mna, ncol=3, nrow=1, labels=c("A)","B)","C)"), font.label = list(size = 14, color = "black", face = "bold"))
}
```
### GD and PD relationship in 5 species with the most number of clusters
...
Ho.lmer.out <- lmer(ho ~ DP_log + (1|sp), data = df_Ho,  REML=F) 
summary(Ho.lmer.out)
```
...
```{r include=TRUE}
He.lmer.out <- lmer(he ~ DP_log + (1|sp), data = df_He, REML=F)
summary(He.lmer.out)
```
...
-   *allelic richness + outliers*

```{r include=TRUE}
mna.lmer.out <- lmer(n.allele_log ~ DP_log + (1|sp), data = df_mna, REML=FALSE)
summary(mna.lmer.out)
```
...
```{r include=TRUE}
Ho.c.lmer.out <- lmer(ho ~ DP_log + (1|sp), data = df_Ho.c,  REML=F) 
summary(Ho.c.lmer.out)
```
...
```{r include=TRUE}
He.c.lmer.out <- lmer(he ~ DP_log + (1|sp), data = df_He.c, REML=F)
summary(He.c.lmer.out)
```
...
```{r include=TRUE}
mna.c.lmer.out <- lmer(n.allele_log ~ DP_log + (1|sp), data = df_mna.c, REML=FALSE)
summary(mna.c.lmer.out)
```
...
```{r, eval=F, include=F}
#Plot figure 2
#source("Script/StatisticalAnalyses_PlotPred.R")
```
...
## PD in cluster vs country
```{r}
tdy <- He.lmer@frame %>% mutate(dataset="cluster")

tdy.c <- He.c.lmer@frame %>% mutate(dataset="country")

tdy.com <- rbind(tdy, tdy.c)
tdy.com$dataset <- as.factor(tdy.com$dataset)
      
ggplot(data=tdy.com, aes(x=sp, y=DP_log, color=dataset, fill=dataset)) + geom_boxplot()

table(tdy.com$sp, tdy.com$dataset)
```

```{r}
ggplot(data=gen_clus, aes(x=he, y=n.allele)) + geom_point()
```
## fitted values by sp
```{r}
plot(Ho.lmer.out, form = resid(., type = "pearson") ~ fitted(.) | sp, abline = 0, 
...
```{r}
library(nlme)
m4.lme = lme(ho ~ DP_log, random = ~1|sp, data = df_Ho, method = "ML")

m5.lme <- update(m4.lme, weights = varIdent(form = ~ 1 | sp), control = lmeControl(msMaxIter = 1000, msMaxEval = 1000))
# compare models
anova(m5.lme, m4.lme)

# observed responses versus the within-group fitted values
plot(m5.lme, ho ~ fitted(.), id = 0.05, adj = -0.3, 
...
```{r}
plot(Ho.lmer.out, id = 0.05, idLabels = ~.obs)

df_Ho[c(179,104,139,146,37,48),]
```
```{r eval=FALSE, include=FALSE}
estex.ho <- influence(Ho.lmer.out, "sp")
#Calculate DFBETAS
dfbetas(estex.ho)
#The cut-off value of DFBETAS equals 2/√n (Belsley et al., 1980), which in this case equals
2/sqrt(14)
...
```{r eval=FALSE, include=FALSE}
#Using cooks distance = how far on average the predicted values will move if the observation is omitted
cooks.distance(estex.ho,
sort=TRUE)

plot(estex.ho, which="cook",
cutoff=4/14, sort=TRUE,
xlab="Cook ́s Distance",
ylab="sp")
```

## Manually
Using all data (including outliers)
The list's name indicate the species data that were left out

### Cluster dataset
*Ho*
```{r}

sp_i <- unique(df_Ho$sp)
res_sp <- list()

for (i in sp_i) {
  res_sp[[i]] <- update(Ho.lmer.out, data=df_Ho[-which(df_Ho$sp == i),])
}

lapply(res_sp, summary)
```
...
sp_i <- unique(df_He$sp)
res_sp <- list()

for (i in sp_i) {
  res_sp[[i]] <- update(He.lmer.out, data=df_He[-which(df_He$sp == i),])
}
lapply(res_sp, summary)
```
...
sp_i <- unique(df_mna$sp)
res_sp <- list()

for (i in sp_i) {
  res_sp[[i]] <- update(mna.lmer.out, data=df_mna[-which(df_mna$sp == i),])
}
lapply(res_sp, summary)
```
...
sp_i <- unique(df_Ho.c$sp)
res_sp <- list()

for (i in sp_i) {
  res_sp[[i]] <- update(Ho.c.lmer.out, data=df_Ho.c[-which(df_Ho.c$sp == i),])
}
...
sp_i <- unique(df_He.c$sp)
res_sp <- list()

for (i in sp_i) {
  res_sp[[i]] <- update(He.c.lmer.out, data=df_He.c[-which(df_He.c$sp == i),])
}
lapply(res_sp, summary)
```
...
sp_i <- unique(df_mna.c$sp)
res_sp <- list()

for (i in sp_i) {
  res_sp[[i]] <- update(mna.c.lmer.out, data=df_mna.c[-which(df_mna.c$sp == i),])
}
lapply(res_sp, summary)
```
### Using species with minimum 5 records

```{r}
xx <- names(which(table(df_Ho$sp) >= 5))
newdf <- df_Ho[df_Ho$sp %in% xx,]
mod.5 <- update(Ho.lmer.out, data = newdf, na.action=na.omit)
summary(mod.5)
res_sp <- list()

for (i in xx) {
  res_sp[[i]] <- update(Ho.lmer.out, data=newdf[-which(newdf$sp == i),])
}
lapply(res_sp, summary)
...
```{r}
xx <- names(which(table(df_mna$sp) >= 5))
newdf <- df_mna[df_mna$sp %in% xx,]
mod.5 <- update(mna.lmer.out, data = newdf, na.action=na.omit)
summary(mod.5)
res_sp <- list()

for (i in xx) {
  res_sp[[i]] <- update(mna.lmer.out, data=newdf[-which(newdf$sp == i),])
}
lapply(res_sp, summary)
...
```{r}
xx <- names(which(table(df_Ho.c$sp) >= 5))
newdf <- df_Ho.c[df_Ho.c$sp %in% xx,]
mod.5 <- update(Ho.c.lmer.out, data = newdf, na.action=na.omit)
summary(mod.5)

res_sp <- list()

for (i in xx) {
  res_sp[[i]] <- update(Ho.c.lmer.out, data=newdf[-which(newdf$sp == i),])
}
lapply(res_sp, summary)
...
```{r}
xx <- names(which(table(df_He.c$sp) >= 5))
newdf <- df_He.c[df_He.c$sp %in% xx,]
mod.5 <- update(He.c.lmer.out, data = newdf, na.action=na.omit)
summary(mod.5)

res_sp <- list()

for (i in xx) {
  res_sp[[i]] <- update(He.c.lmer.out, data=newdf[-which(newdf$sp == i),])
}
lapply(res_sp, summary)
...
```{r}
xx <- names(which(table(df_mna.c$sp) >= 5))
newdf <- df_mna.c[df_mna.c$sp %in% xx,]
mod.5 <- update(mna.c.lmer.out, data = newdf, na.action=na.omit)
summary(mod.5)

res_sp <- list()

for (i in xx) {
  res_sp[[i]] <- update(mna.c.lmer.out, data=newdf[-which(newdf$sp == i),])
}
lapply(res_sp, summary)
...
```{r}
new2df <- df_mna.c[!df_mna.c$sp %in% c("Panthera tigris","Panthera pardus"),]
mod.2 <- update(mna.c.lmer.out, data = new2df, na.action=na.omit)
summary(mod.2)
```
